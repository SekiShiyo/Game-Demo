/*:
 * @target MZ
 * @plugindesc 自定义列表商店系统 v4.0.0 - Scene模式 by Seki
 * @author Seki
 * @help
 * 
 * 这是一个完全自定义的商店系统，使用独立场景实现，完全模拟原生商店行为。
 * 支持商店配置、物品分类、自定义UI等功能。
 * 
 * 批量物品配置说明：
 * - 单个ID: "5"
 * - 连续范围: "1-10" 
 ...
 * @text 通知窗口设置
 * 
 * @param notificationWidth
 * @text 通知窗口宽度
 * @parent notificationSettings
 * @type number
 * @min 200
 * @max 800
 * @desc 通知窗口的宽度（-1表示自动适应商店宽度）
 * @default -1
 * 
 ...
 * @default 120
 * 
 * @param successNotificationBgColor
 * @text 成功通知背景色
 * @parent notificationSettings
 * @type string
 * @desc 购买/出售成功时的通知背景颜色
 * @default #ddffdd
 * 
 * @param successNotificationBorderColor
 * @text 成功通知边框色
 * @parent notificationSettings
 * @type string
 * @desc 购买/出售成功时的通知边框颜色
 * @default #00aa00
 * 
 * @param successNotificationTextColor
 * @text 成功通知字体色
 * @parent notificationSettings
 * @type string
 * @desc 购买/出售成功时的通知字体颜色
 * @default #006600
 * 
 * @param errorNotificationBgColor
 * @text 错误通知背景色
 * @parent notificationSettings
 * @type string
 * @desc 余额不足等错误时的通知背景颜色
 * @default #ffdddd
 * 
 * @param errorNotificationBorderColor
 * @text 错误通知边框色
 * @parent notificationSettings
 * @type string
 * @desc 余额不足等错误时的通知边框颜色
 * @default #ff0000
 * 
 * @param errorNotificationTextColor
 * @text 错误通知字体色
 * @parent notificationSettings
 * @type string
 * @desc 余额不足等错误时的通知字体颜色
 * @default #cc0000
 * 
 * @param notificationFontSize
 * @text 通知字体大小
 * @parent notificationSettings
 * @type number
 * @min 12
...
(() => {
    ...
    // 解析通知窗口设置参数
    const notificationWidth = Number(parameters['notificationWidth'] || -1);
    const notificationHeight = Number(parameters['notificationHeight'] || 80);
    const notificationX = Number(parameters['notificationX'] || -1);
    const notificationY = Number(parameters['notificationY'] || -1);
    const notificationDuration = Number(parameters['notificationDuration'] || 120);
    const successNotificationBgColor = String(parameters['successNotificationBgColor'] || '#ddffdd');
    const successNotificationBorderColor = String(parameters['successNotificationBorderColor'] || '#00aa00');
    const successNotificationTextColor = String(parameters['successNotificationTextColor'] || '#006600');
    const errorNotificationBgColor = String(parameters['errorNotificationBgColor'] || '#ffdddd');
    const errorNotificationBorderColor = String(parameters['errorNotificationBorderColor'] || '#ff0000');
    const errorNotificationTextColor = String(parameters['errorNotificationTextColor'] || '#cc0000');
    const notificationFontSize = Number(parameters['notificationFontSize'] || 18);
    ...
    Scene_CustomShop.prototype.initialize = function() {
        ...
        // 添加累积统计变量
        this._totalSpent = 0;     // 总消费
        this._totalEarned = 0;    // 总收入
    };
    ...
    Scene_CustomShop.prototype.createNotificationWindow = function() {
        // 计算通知窗口大小
        const finalNotificationWidth = notificationWidth === -1 ? windowWidth - 40 : notificationWidth;
        const finalNotificationHeight = notificationHeight;
        
        // 计算通知窗口位置
        let finalNotificationX, finalNotificationY;
        
        if (notificationX === -1) {
            // 居中
            finalNotificationX = this._panelX + (windowWidth - finalNotificationWidth) / 2;
        } else {
            finalNotificationX = this._panelX + notificationX;
        }
        
        if (notificationY === -1) {
            // 默认底部位置
            finalNotificationY = this._panelY + windowHeight - finalNotificationHeight - 20;
        } else {
            finalNotificationY = this._panelY + notificationY;
        }
        
        // 创建通知窗口
        this._notificationWindow = new Sprite();
        this._notificationWindow.bitmap = new Bitmap(finalNotificationWidth, finalNotificationHeight);
        this._notificationWindow.x = finalNotificationX;
        this._notificationWindow.y = finalNotificationY;
        this._notificationWindow.visible = false;
        this._notificationTimer = 0;
        this.addChild(this._notificationWindow);
    };
    
    Scene_CustomShop.prototype.showNotification = function(message, isError = false) {
        this._notificationWindow.bitmap.clear();
        
        // 根据类型选择颜色
        let bgColor, borderColor, textColor;
        if (isError) {
            bgColor = errorNotificationBgColor;
            borderColor = errorNotificationBorderColor;
            textColor = errorNotificationTextColor;
        } else {
            bgColor = successNotificationBgColor;
            borderColor = successNotificationBorderColor;
            textColor = successNotificationTextColor;
        }
        
        // 绘制背景
        this._notificationWindow.bitmap.fillRect(0, 0, this._notificationWindow.bitmap.width, this._notificationWindow.bitmap.height, bgColor);
        this._notificationWindow.bitmap.strokeRect(0, 0, this._notificationWindow.bitmap.width, this._notificationWindow.bitmap.height, borderColor);
        
        // 绘制文本
        this._notificationWindow.bitmap.fontFace = $gameSystem.mainFontFace();
        this._notificationWindow.bitmap.fontSize = notificationFontSize;
        this._notificationWindow.bitmap.textColor = textColor;
        this._notificationWindow.bitmap.drawText(
            message, 
            10, 
            (this._notificationWindow.bitmap.height - notificationFontSize) / 2, 
            this._notificationWindow.bitmap.width - 20, 
            notificationFontSize, 
            'center'
        );
        
        this._notificationWindow.visible = true;
        this._notificationTimer = notificationDuration;
    };
    ...
    Scene_CustomShop.prototype.processBuy = function(item) {
        if ($gameParty.gold() >= item.finalPrice) {
            ...
            // 累积统计
            this._totalSpent += item.finalPrice;
            
            // 显示购买成功提示
            this.showNotification(`购买了 ${item.itemObject.name}！`, false);
        } else {
            // 显示余额不足提示（在商店内立即显示）
            this.showNotification('余额不足！', true);
    ...
    Scene_CustomShop.prototype.processSell = function(item) {
        if ($gameParty.numItems(item.itemObject) > 0) {
            ...
            // 累积统计
            this._totalEarned += item.finalPrice;
            
            // 显示出售成功提示
            this.showNotification(`出售了 ${item.itemObject.name}！`, false);
        } else {
            // 显示物品不足提示（在商店内立即显示）
            this.showNotification('没有这个物品可以出售！', true);
            SoundManager.playBuzzer();
        }
    };
    ...
    Scene_CustomShop.prototype.updateNotification = function() {
    ...
    Scene_CustomShop.prototype.popScene = function() {
        // 在关闭商店时显示总计提示
        if (this._totalSpent > 0) {
            $gameMessage.add(`总计消费了【${this._totalSpent}】${$dataSystem.currencyUnit}`);
        }
        if (this._totalEarned > 0) {
            $gameMessage.add(`总计获得了【${this._totalEarned}】${$dataSystem.currencyUnit}`);
        }
