 ...
 * @default 0
 * 
 * @param taskListFontColor
 * @type number
 * @text 任务列表字体颜色
 * @desc 任务列表中任务名称的字体颜色(0-31的系统颜色索引)
 * @default 0
 *
 * @param debugMode
 * @type boolean
...
(function() {
    // 插件名称常量
    const PLUGIN_NAME = "ExtremeSimpleTaskSystem";
    ...
    TaskSystem.settings = {
        ...
        taskListFontColor: Number(parameters['taskListFontColor'] || 0),
        ...
        windowOpacity: Number(parameters['windowOpacity'] || 0)
    };
    ...
    TaskDetailWindow.prototype.initialize = function(rect) {
        ...
        this._task = null;
        this.opacity = TaskSystem.settings.windowOpacity || 0;
    ...
    TaskListWindow.prototype.drawItem = function(dataIndex, visibleIndex) {
        ...
        if (task.status === 'completed') {
            ...
            this.changeTextColor(ColorManager.textColor(TaskSystem.settings.taskListFontColor));
        }
    ...
    Scene_Task.prototype.createBackground = function() {
        ...
        if (this._backgroundSprite) {
            this._backgroundSprite.opacity = 128; // 降低不透明度让背景变暗
        }
        ...
        if (TaskSystem.settings.useAnimatedBackground) {
            ...
            this._backgroundSprite.scale.x = TaskSystem.settings.bgScaleX;
            this._backgroundSprite.scale.y = TaskSystem.settings.bgScaleY;
            this._backgroundSprite.opacity = TaskSystem.settings.bgOpacity;
